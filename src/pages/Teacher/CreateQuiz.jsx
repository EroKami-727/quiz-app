...
const CreateQuiz = () => {
    ...
    const handleEditCrop = (target) => {
        ...
        if (fileToEdit) {
            if (fileToEdit.url) {
                // To re-crop an uploaded image, we must fetch it first
    ...
    const handleFile = (file, cropData, target) => {
        updateQuestionState(target.qIndex, q => {
            ...
            if (target.field === 'questionMedia') { setMedia(newQ); }
            else if (target.field === 'mcqOptionMedia') { newQ.mcqData = { ...q.mcqData, options: q.mcqData.options.map((opt, i) => i === target.oIndex ? { ...opt, localMediaFile: file, media: null, localCropData: cropData } : opt) }; }
            else if (target.field === 'matchPromptMedia') { newQ.matchData = { ...q.matchData, pairs: q.matchData.pairs.map((p, i) => i === target.pairIndex ? { ...p, promptLocalMediaFile: file, promptMedia: null, promptLocalCropData: cropData } : p) }; }
            else if (target.field === 'matchAnswerMedia') { newQ.matchData = { ...q.matchData, pairs: q.matchData.pairs.map((p, i) => i === target.pairIndex ? { ...p, answerLocalMediaFile: file, answerMedia: null, answerLocalCropData: cropData } : p) }; }
            else if (target.field === 'categorizeItemMedia') { newQ.categorizeData = { ...q.categorizeData, items: q.categorizeData.items.map((item, i) => i === target.itemIndex ? { ...item, localMediaFile: file, media: null, localCropData: cropData } : item) }; }
            else if (target.field === 'reorderItemMedia') { newQ.reorderData = { ...q.reorderData, items: q.reorderData.items.map((item, i) => i === target.itemIndex ? { ...item, localMediaFile: file, media: null, localCropData: cropData } : item) }; }
            else if (target.field === 'visualMainMedia') { newQ.visualData = { ...q.visualData, localMainMediaFile: file, mainMedia: null, localCropData: cropData }; }
            else if (target.field === 'listeningMainMedia') { newQ.listeningData = { ...q.listeningData, localMainMediaFile: file, mainMedia: null, localCropData: null }; }
            return newQ;
        });
    };
    ...
    const handlePublishQuiz = async () => {
        ...
        try {
            ...
            const processUploads = (questionSet, questionSetToSave) => {
                questionSet.forEach((q, qIndex) => {
                    const addUpload = (file, cropData, target, prop) => {
                        uploadPromises.push(imagekit.upload({ file, fileName: `quiz-media_${Date.now()}_${file.name}`, folder: '/quiz-app-media' }).then(result => {
            ...
            setIsPublished(true);
        } catch (e) { console.error("Error publishing:", e); setError('An error occurred. Please check console and try again.'); } 
                                ...
                                <div key={q.id} className="question-card">
                                    ...
                                    {(q.type === 'VISUAL_COMPREHENSION' || q.type === 'LISTENING_COMPREHENSION') ? (
                                      <div className="comprehension-container">
                                        ...
                                        <div className="main-media-controls full-width">{(q.visualData?.mainMedia || q.visualData?.localMainMediaFile || q.listeningData?.mainMedia || q.listeningData?.localMainMediaFile) ? <MediaPreview file={q.visualData?.mainMedia || q.visualData?.localMainMediaFile || q.listeningData?.mainMedia || q.listeningData.localMainMediaFile} cropData={q.visualData?.localCropData || q.visualData?.media?.cropData} onRemove={() => handleRemoveMedia(qIndex, q.type === 'VISUAL_COMPREHENSION' ? 'visualMainMedia' : 'listeningMainMedia')} onEdit={() => handleEditCrop({ qIndex, field: q.type === 'VISUAL_COMPREHENSION' ? 'visualMainMedia' : 'listeningMainMedia' })}/> : <button type="button" className="add-media-btn" onClick={() => openUploadModal({ qIndex, field: q.type === 'VISUAL_COMPREHENSION' ? 'visualMainMedia' : 'listeningMainMedia' })}><FaPhotoVideo/> Add Media</button>}</div>
